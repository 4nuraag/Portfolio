...
export default function ProjectOverlay({ project, onClose }: ProjectOverlayProps) {
    // Lock body scroll and close on navigation
    useEffect(() => {
        if (project) {
            ...
            {project && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed top-4 left-4 right-4 bottom-24 md:inset-auto md:top-[10%] md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-5xl md:h-auto md:max-h-[85vh] bg-[#0A0A0A] border border-white/10 rounded-3xl shadow-2xl z-50 overflow-hidden flex flex-col"
                    >
                        {/* Header Actions */}
                        ...
                        <div className="flex flex-col md:flex-row h-full overflow-y-auto md:overflow-hidden">
                            ...
                            <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col overflow-y-auto">
                                ...
                                <div className="space-y-6 mt-auto">
                                    {/* Skills / Tags */}
                                    <div>
                                        <h4 className="text-white/40 text-xs font-bold uppercase tracking-widest mb-3">Skills & Technologies</h4>
                                        <div className="flex flex-wrap gap-2">
                                            {project.tags ? project.tags.map(tag => (
                                                ...
                                                <p className="text-gray-200 text-sm">{project.skills}</p>
                                            )}
                                        </div>
                                    </div>
